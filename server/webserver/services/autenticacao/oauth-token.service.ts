...
export class OAuthTokenService {
  ...
  public criarAcessToken( usuario: Usuario ): OAuthTokenResponseDTO {
    ...
    const tokenCripto: AccessTokenJwt = {
      tipo : 'U',
  ...
  public getAccessTokenValido(req: Req): AccessTokenJwt {
    if ( req.headers.authorization.startsWith('Bearer ') ) {
      const accessToken = req.headers.authorization.split(' ')[1];
      try {
        return jwt.verify(accessToken, config.oauthJwt.JWT_PW);
      }
      catch (e) {
        console.error('O token de acesso não está válido');
      }
    }
  ...
  public usuarioTemPermissao(accessToken: AccessTokenJwt, permissoesAValidar: ChavePermissao[]): boolean {
    ...
    return permissoesAValidar.every(permissaoAValidar => accessToken.authorities.includes(permissaoAValidar));
  }

  @Deprecated('Utilize this.oauthTokenService.getAccessTokenValido(req) e depois this.oauthTokenService.usuarioTemPermissao() ')
  public getPermissoesDaRequest(req: Req): ChavePermissao[] {
    if ( req.headers.authorization.startsWith('Bearer ') ) {
      const accessToken = req.headers.authorization.split(' ')[1];
      try {
        const token = jwt.verify(accessToken, config.oauthJwt.JWT_PW);
